...
do
	...
	do
		local function __modImpl()
			return {
				Chaotic = __DARKLUA_BUNDLE_MODULES.l(),
			}
		end
	...
	do
		local function __modImpl()
			local Step = __DARKLUA_BUNDLE_MODULES.n()
			local Ast = __DARKLUA_BUNDLE_MODULES.b()
			local Scope = __DARKLUA_BUNDLE_MODULES.i()
			local AstKind = Ast.AstKind
	...
	do
		local function __modImpl()
			...
			local Ast = __DARKLUA_BUNDLE_MODULES.b()
			local Scope = __DARKLUA_BUNDLE_MODULES.i()
			local RandomStrings = __DARKLUA_BUNDLE_MODULES.p()
			local Parser = __DARKLUA_BUNDLE_MODULES.j()
			local Enums = __DARKLUA_BUNDLE_MODULES.g()
			local logger = __DARKLUA_BUNDLE_MODULES.d()
			local visitast = __DARKLUA_BUNDLE_MODULES.q()
			local util = __DARKLUA_BUNDLE_MODULES.f()
			local AstKind = Ast.AstKind
            ...
            do
            	...
            	local ]] .. realstrings_key .. [[ = {}
            	STRINGS = setmetatable({}, {
            		__index = ]] .. realstrings_key .. [[,
            		__metatable = nil,
            	})
            ...
            do
            	...
            	local ]] .. localcache_key .. [[={}
            	local function ]] .. locdec_key .. [[(str,seed)
            		if not ]] .. localcache_key .. [[[seed] then
            			...
            			]] .. localcache_key .. [[[seed]=table.concat(parts)
            		end
            		return seed
            	end
            	STRINGS = setmetatable({}, {
            		__index = function(_, k) return ]] .. localcache_key .. [[[k] end,
            		__metatable = nil,
            	})
            	function DECRYPT(s,k) return ]] .. locdec_key .. [[(s,k) end
            end]]
						return code
					else
						local code = [[
            do
            	...
            	]] .. tbl_key .. [[.]] .. dec_key .. [[=function(str,seed)
            		if not ]] .. tbl_key .. [[.]] .. cache_key .. [[[seed] then
            			...
            			]] .. tbl_key .. [[.]] .. cache_key .. [[[seed]=table.concat(p)
            		end
            		return seed
            	end
            	STRINGS=setmetatable({},{
            		__index=function(_,k) return ]] .. tbl_key .. [[.]] .. cache_key .. [[[k] end,
            		__metatable=nil,
            	})
            	function DECRYPT(s,k) return ]] .. tbl_key .. [[.]] .. dec_key .. [[(s,k) end
            end]]
						return code
					end
				end
	...
	do
		local function __modImpl()
			local Step = __DARKLUA_BUNDLE_MODULES.n()
			local Compiler = __DARKLUA_BUNDLE_MODULES.A()
			local Optimizer = __DARKLUA_BUNDLE_MODULES.B()
	...
	do
		local function __modImpl()
			...
			local function flattenIfNode(node)
				...
				local body = node.body
				local hasReturn = body
					and body.statements
					and body.statements[#body.statements]
			...
			local function flattenFunctionNode(node)
				...
				local xorKey = math.random(1, 2 ^ 28)
				local function xorOp(v)
					local r, a, b, m = 0, v, xorKey, 1
					while a > 0 or b > 0 do
						if (a % 2) ~= (b % 2) then r = r + m end
						a = math.floor(a / 2)
						b = math.floor(b / 2)
				...
				local keyName = "_k" .. tostring(math.random(100000, 999999))
				local keyDecl = {
					kind = AstKind.LocalVariableDeclaration,
					variables = { { kind = AstKind.Identifier, name = keyName } },
					values = { { kind = AstKind.NumberExpression, value = xorKey } },
				}
				local stateDecl = {
					kind = AstKind.LocalVariableDeclaration,
					variables = { { kind = AstKind.Identifier, name = "state" } },
					values = { { kind = AstKind.NumberExpression, value = xorOp(firstOp) } },
				}
				local hoistDecl = nil
				...
				local function buildIfChain(entries, i)
					...
					elseif dispatchStmt.kind == AstKind.LocalFunctionDeclaration then
						dispatchStmt = {
							kind = AstKind.AssignmentStatement,
					...
					local stateAssign = {
						kind = AstKind.AssignmentStatement,
						variables = { { kind = AstKind.Identifier, name = "state" } },
						values = { { kind = AstKind.NumberExpression, value = xorOp(entry.next) } },
					}
					local ifNode = {
						kind = AstKind.IfStatement,
						condition = {
							...
							right = { kind = AstKind.NumberExpression, value = xorOp(entry.op) },
						},
						body = {
							kind = AstKind.Block,
				...
				local newStmts = { keyDecl, stateDecl }
				if hoistDecl then
				...
				visitast(ast, function(node, data)
					...
					while i <= #node.statements do
						...
						if not UNSAFE_WRAP[stmt.kind] and math.random() <= treshold then
							...
							local whileNode = Ast.WhileStatement(loopBody, Ast.BooleanExpression(true), node.scope)
	...
	do
		local function __modImpl()
			return {
				InvertedExecution = __DARKLUA_BUNDLE_MODULES.o(),
				EncryptStrings = __DARKLUA_BUNDLE_MODULES.r(),
				SplitStrings = __DARKLUA_BUNDLE_MODULES.s(),
				AntiTamper = __DARKLUA_BUNDLE_MODULES.t(),
				DynamicXOR = __DARKLUA_BUNDLE_MODULES.u(),
				DynamicDecrypt = __DARKLUA_BUNDLE_MODULES.v(),
				DynamicJumps = __DARKLUA_BUNDLE_MODULES.w(),
				ConstantsObfuscator = __DARKLUA_BUNDLE_MODULES.x(),
				NumbersToExpressions = __DARKLUA_BUNDLE_MODULES.y(),
				OpaquePredicates = __DARKLUA_BUNDLE_MODULES.z(),
				Vmify = __DARKLUA_BUNDLE_MODULES.C(),
				StatementFlattener = __DARKLUA_BUNDLE_MODULES.D(),
				AntiDump = __DARKLUA_BUNDLE_MODULES.E(),
				VirtualGlobals = __DARKLUA_BUNDLE_MODULES.F(),
				FakeLoopWrap = __DARKLUA_BUNDLE_MODULES.G(),
				WrapInFunction = __DARKLUA_BUNDLE_MODULES.H(),
				Compressor = __DARKLUA_BUNDLE_MODULES.I(),
				DeadCodeEliminator = __DARKLUA_BUNDLE_MODULES.J(),
			}
		end
		function __DARKLUA_BUNDLE_MODULES.K()
			local v = __DARKLUA_BUNDLE_MODULES.cache.K
	...
	do
		local function __modImpl()
			...
			local logger = __DARKLUA_BUNDLE_MODULES.d()
			local NameGenerators = __DARKLUA_BUNDLE_MODULES.m()
			local Steps = __DARKLUA_BUNDLE_MODULES.K()
			local lookupify = util.lookupify
			local LuaVersion = Enums.LuaVersion
			local AstKind = Ast.AstKind
		...
		local function __modImpl()
			return {
				["Luraph"] = {
					...
					Steps = {
						{
							Name = "InvertedExecution",
							Settings = { ChunkSize = 8, RandomiseIds = true, JunkChunks = 6 },
						},
						...
						{
							Name = "DynamicJumps",
							Settings = { Threshold = 1.0, PoolSize = 256, MinStatements = 1 },
						},
						{
						...
						{ Name = "NumbersToExpressions", Settings = { Treshold = 0.2, InternalTreshold = 0.1 } },
	...
	do
		local function __modImpl()
			...
			local Pipeline = __DARKLUA_BUNDLE_MODULES.L()
			local highlight = __DARKLUA_BUNDLE_MODULES.M()
			local colors = __DARKLUA_BUNDLE_MODULES.c()
			local Logger = __DARKLUA_BUNDLE_MODULES.d()
			local Presets = __DARKLUA_BUNDLE_MODULES.N()
			local Config = __DARKLUA_BUNDLE_MODULES.a()
			local util = __DARKLUA_BUNDLE_MODULES.f()
			return {
				Pipeline = Pipeline,
...
local ZukaTech = __DARKLUA_BUNDLE_MODULES.O()
if arg and arg[0] then
	local inputFile  = arg[1]
	local outputFile = arg[2]
	if not inputFile then
		io.stderr:write("Usage: lua ZukaTechFull.lua <input.lua> [output.lua]\n")
		os.exit(1)
	end
	local f, err = io.open(inputFile, "r")
	if not f then
	...
	f:close()
	local pipeline = ZukaTech.Pipeline:fromConfig(ZukaTech.Presets["Luraph"])
	local ok, result = pcall(function()
