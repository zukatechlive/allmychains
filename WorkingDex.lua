...
local function startLivePulse()
	...
	task.spawn(function()
		while liveBadge and liveBadge.Parent do
			for t = 0, 1, 0.05 do
				if not (liveBadge and liveBadge.Parent) then
					return
				end
...
do
	...
	do
		...
		local luaSet = makeSet(keywords_lua)
		local rbxSet = makeSet(keywords_rbx)
		local exploitSet = makeSet(keywords_exploit)
		local opSet = makeSet(operators)
	...
	local function buildConsoleUI()
		...
		mk("UICorner", { CornerRadius = UDim.new(0, 3) }, consoleToggleBtn)
